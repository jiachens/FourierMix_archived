...
Description: 
Autor: Jiachen Sun
Date: 2021-07-26 18:56:10
LastEditors: Jiachen Sun
LastEditTime: 2021-07-26 18:56:13
'''
...
parser.add_argument('--objective', default='denoising', type=str,
                    ...
                    choices=['denoising', 'classification', 'stability', 'both', 'finetune'])
...
def main():
    ...
    if args.objective == 'denoising':
    ...
    elif args.objective in ['classification','stability','both']:
        ...
        if args.objective == 'both':
            clf.cuda().train()
            if args.optimizer == 'Adam':
                optimizer = Adam(list(denoiser.parameters())+list(clf.parameters()), lr=args.lr, weight_decay=args.weight_decay)
            elif args.optimizer == 'SGD':
                optimizer = SGD(list(denoiser.parameters())+list(clf.parameters()), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
            elif args.optimizer == 'AdamThenSGD':
                optimizer = Adam(list(denoiser.parameters())+list(clf.parameters()), lr=args.lr, weight_decay=args.weight_decay)
            scheduler = StepLR(optimizer, step_size=args.lr_step_size, gamma=args.gamma)
        else:
    ...
    elif args.objective in ['finetune']:
        assert args.classifier != '', "Please specify a path to the classifier you want to attach the denoiser to."
        ...
        if args.classifier in IMAGENET_CLASSIFIERS:
            ...
            clf = base_classifier
        if args.optimizer == 'Adam':
            optimizer = Adam(clf.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        elif args.optimizer == 'SGD':
            optimizer = SGD(clf.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        elif args.optimizer == 'AdamThenSGD':
            optimizer = Adam(clf.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        scheduler = StepLR(optimizer, step_size=args.lr_step_size, gamma=args.gamma)
        clf.cuda().train()
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        if args.objective != 'denoising':
            ...
            }, os.path.join(args.outdir, 'clf_checkpoint.pth.tar'))
